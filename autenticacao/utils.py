...
# Função enviar email e criar pasta html em templates da pasta autentificação
def email_html(path_template: str, assunto: str, para: list, **kwargs) -> dict:
    
    html_content = render_to_string(path_template, kwargs)  # Renderiza o arquivo html em algo que o email entenda
    text_content = strip_tags(html_content)  # Remove as tags html
    
    email = EmailMultiAlternatives(assunto, text_content, settings.EMAIL_HOST_USER, para)  # Transforma tudo em algo que o django consiga entender
